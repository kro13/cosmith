...
var kro13_cosmith_data_GameData = function() {
};
kro13_cosmith_data_GameData.__name__ = true;
kro13_cosmith_data_GameData.get_instance = function() {
	if(kro13_cosmith_data_GameData.instance == null) {
		kro13_cosmith_data_GameData.instance = new kro13_cosmith_data_GameData();
	}
	return kro13_cosmith_data_GameData.instance;
};
kro13_cosmith_data_GameData.prototype = {
	map: null
	,init: function(map) {
		this.map = new kro13_cosmith_data_scopes_MapData(map);
	}
	,__class__: kro13_cosmith_data_GameData
};
var kro13_cosmith_data_GameDataFactory = function() {
};
kro13_cosmith_data_GameDataFactory.__name__ = true;
kro13_cosmith_data_GameDataFactory.get_instance = function() {
	if(kro13_cosmith_data_GameDataFactory.instance == null) {
		kro13_cosmith_data_GameDataFactory.instance = new kro13_cosmith_data_GameDataFactory();
	}
	return kro13_cosmith_data_GameDataFactory.instance;
};
kro13_cosmith_data_GameDataFactory.prototype = {
	newMap: function(width,height) {
		var map = this.newGameObject(-1);
		map.w = width;
		map.h = height;
		map.image = "images/map_test.jpg";
		map.objects = [];
		map.name = "Map";
		return map;
	}
	,newGameObject: function(id,type) {
		if(type == null) {
			type = 0;
		}
		var template = { id : id, type : type, image : "images/go_stub.png", x : 0, y : 0, w : 0, h : 0, name : ""};
		switch(type) {
		case 1:
			this.setupPawn(template);
			break;
		case 2:
			this.setupHero(template);
			break;
		case 3:
			this.setupNPC(template);
			break;
		default:
		}
		return template;
	}
	,setupPawn: function(tmpl) {
		tmpl.w = tmpl.h = 5;
		tmpl.name = "Pawn" + tmpl.id;
	}
	,setupNPC: function(tmpl) {
		tmpl.w = tmpl.h = 2;
		tmpl.name = "NPC" + tmpl.id;
	}
	,setupHero: function(tmpl) {
		tmpl.w = tmpl.h = 3;
		tmpl.name = "Hero" + tmpl.id;
	}
	,__class__: kro13_cosmith_data_GameDataFactory
};
var kro13_cosmith_data_scopes_MapData = function(data) {
	this.data = data;
	this.objects = data.objects;
	this.buildIndex();
};
kro13_cosmith_data_scopes_MapData.__name__ = true;
kro13_cosmith_data_scopes_MapData.prototype = {
	data: null
	,objects: null
	,idsToObjects: null
	,addObject: function(object) {
		this.objects.push(object);
		this.idsToObjects.h[object.id] = object;
	}
	,removeObject: function(object) {
		HxOverrides.remove(this.objects,object);
		this.idsToObjects.remove(object.id);
	}
	,objectExists: function(id) {
		return this.idsToObjects.h.hasOwnProperty(id);
	}
	,getObjectById: function(id) {
		return this.idsToObjects.h[id];
	}
	,getObjectsOnSameTile: function(object) {
		var result = [];
		var _g = 0;
	...
	,buildIndex: function() {
		this.idsToObjects = new haxe_ds_IntMap();
		var _g = 0;
		var _g1 = this.objects;
		while(_g < _g1.length) {
			var obj = _g1[_g];
			++_g;
			this.idsToObjects.h[obj.id] = obj;
		}
	}
	,printObjects: function() {
		...
		while(_g < _g1.length) {
			var obj = _g1[_g];
			++_g;
			console.log("src/kro13/cosmith/data/scopes/MapData.hx:71:",obj);
		}
	}
	,__class__: kro13_cosmith_data_scopes_MapData
};
var kro13_cosmith_data_types_ECommand = $hxEnums["kro13.cosmith.data.types.ECommand"] = { __ename__ : true, __constructs__ : ["NONE","SPAWN","MOVE"]
	,NONE: {_hx_index:0,__enum__:"kro13.cosmith.data.types.ECommand",toString:$estr}
	,SPAWN: ($_=function(id,type,x,y,name) { return {_hx_index:1,id:id,type:type,x:x,y:y,name:name,__enum__:"kro13.cosmith.data.types.ECommand",toString:$estr}; },$_.__params__ = ["id","type","x","y","name"],$_)
	,MOVE: ($_=function(id,x,y) { return {_hx_index:2,id:id,x:x,y:y,__enum__:"kro13.cosmith.data.types.ECommand",toString:$estr}; },$_.__params__ = ["id","x","y"],$_)
};
var kro13_cosmith_data_types_EMessageType = $hxEnums["kro13.cosmith.data.types.EMessageType"] = { __ename__ : true, __constructs__ : ["USER","COMMAND"]
	,USER: {_hx_index:0,__enum__:"kro13.cosmith.data.types.EMessageType",toString:$estr}
	,COMMAND: ($_=function(command) { return {_hx_index:1,command:command,__enum__:"kro13.cosmith.data.types.EMessageType",toString:$estr}; },$_.__params__ = ["command"],$_)
};
var kro13_cosmith_server_CosmithServer = function() { };
...
kro13_cosmith_server_CosmithServer.initData = function() {
	var factory = kro13_cosmith_data_GameDataFactory.get_instance();
	kro13_cosmith_data_GameData.get_instance().init(factory.newMap(100,100));
	var npc1 = factory.newGameObject(kro13_cosmith_server_Storage.goIds++,3);
	npc1.x = 20;
	npc1.y = 10;
	kro13_cosmith_data_GameData.get_instance().map.addObject(npc1);
	var npc2 = factory.newGameObject(kro13_cosmith_server_Storage.goIds++,3);
	npc2.x = 10;
	npc2.y = 20;
	kro13_cosmith_data_GameData.get_instance().map.addObject(npc2);
};
kro13_cosmith_server_CosmithServer.startMessenger = function() {
...
kro13_cosmith_server_Root.__name__ = true;
kro13_cosmith_server_Root.prototype = {
	socketServer: null
	,root: function() {
	...
	,__class__: kro13_cosmith_server_Root
};
var kro13_cosmith_server_Storage = function() { };
kro13_cosmith_server_Storage.__name__ = true;
...
tink_Chunk.join = function(chunks) {
	if(chunks == null) {
		...
		default:
			var v = chunks;
			var ret = tink_Chunk.catChunk(v[0],v[1]);
			var _g = 2;
			var _g1 = v.length;
			while(_g < _g1) {
				var i = _g++;
				ret = tink_Chunk.catChunk(ret,v[i]);
			}
			return ret;
		}
	}
...
tink_Chunk.rcatString = function(a,b) {
	return tink_Chunk.catChunk(a,tink_chunk_ByteChunk.of(haxe_io_Bytes.ofString(b)));
};
tink_Chunk.lcatString = function(a,b) {
	return tink_Chunk.catChunk(tink_chunk_ByteChunk.of(haxe_io_Bytes.ofString(a)),b);
};
tink_Chunk.lcatBytes = function(a,b) {
	return tink_Chunk.catChunk(tink_chunk_ByteChunk.of(a),b);
};
tink_Chunk.rcatBytes = function(a,b) {
	return tink_Chunk.catChunk(a,tink_chunk_ByteChunk.of(b));
};
...
tink_http_ChunkedEncoder.prototype = {
	transform: function(source) {
		return tink_io_Source.chunked(source).map(tink_streams_Mapping.ofPlain(function(chunk) {
			return tink_Chunk.catChunk(tink_Chunk.catChunk(tink_chunk_ByteChunk.of(haxe_io_Bytes.ofString("" + StringTools.hex(chunk.getLength()) + "\r\n")),chunk),tink_chunk_ByteChunk.of(haxe_io_Bytes.ofString("\r\n")));
...
tink_http_UploadedFile.ofBlob = function(name,type,data) {
	return { fileName : name, mimeType : type, size : data.length, read : function() {
		...
		var ret = tink_io_Source.pipeTo(new tink_streams_Single(new tink_core__$Lazy_LazyConst(tink_chunk_ByteChunk.of(data))),dest,{ end : true}).map(function(r) {
			switch(r._hx_index) {
			case 0:
...
tink_http_containers_NodeContainer.toNodeHandler = function(handler,options) {
	...
	return function(req,res) {
		handler.process(new tink_http_IncomingRequest(req.socket.remoteAddress,tink_http_IncomingRequestHeader.fromIncomingMessage(req),body(req))).handle(function(out) {
...
tink_http_containers_NodeContainer.prototype = {
	...
	,run: function(handler) {
		...
		return tink_core_Future.async(function(cb) {
			...
			server.on("request",tink_http_containers_NodeContainer.toNodeHandler(handler));
			server.on("error",function(e) {
				cb(tink_http_ContainerResult.Failed(e));
			});
		});
...
tink_io_Splitter.prototype = $extend(tink_io_BytewiseParser.prototype,{
	...
	,read: function(char) {
		...
		this.buf = tink_Chunk.catChunk(this.buf,tink_chunk_ByteChunk.of(haxe_io_Bytes.ofString(String.fromCodePoint(char))));
...
tink_io_nodejs_NodejsSink.prototype = $extend(tink_io_SinkBase.prototype,{
	...
	,consume: function(source,options) {
		...
		var ret = source.forEach(tink_streams_Handler.ofUnknown(function(c) {
			var ret = _gthis.target.write(c).map(function(w) {
				switch(w._hx_index) {
				case 0:
					if(w.data) {
						return tink_streams_Handled.Resume;
...
tink_io_nodejs_WrappedWritable.prototype = {
	...
	,end: function() {
		var didEnd = false;
		var this1 = this.ended.handle(tink_core_Callback.fromNiladic(function() {
			didEnd = true;
		}));
		if(this1 != null) {
			this1.cancel();
	...
	process0: function() {
		...
		if(!tmp) {
			while(true) {
				...
				var tmp1;
				if("name".length == __name__.max - __name__.min) {
					var max = __name__.max;
					tmp1 = __name__.source.startsWith("name",__name__.min);
				} else {
					tmp1 = false;
				}
				if(tmp1) {
					v__0 = tink_json__$Parser_JsonString.toString(this.parseString());
					hasv__0 = true;
				} else {
					this.skipValue();
				}
		...
		return { _0 : hasv__0 ? v__0 : __missing__("name")};
	}
	...
	,tryParse: function(input) {
		try {
			...
			if(((_g1) instanceof tink_core_TypedError)) {
				...
				var e = _g1;
				return tink_core_Outcome.Failure(this.error("Parse Error",e));
			}
		}
	}
...
tink_querystring_Parser0.prototype = $extend(tink_querystring_ParserBase.prototype,{
	...
	,process0: function(prefix) {
		...
		var __o = { _0 : this.exists.h[prefix1] ? tink_web_forms_FormField.toString(this.params.h[prefix1]) : this.missing(prefix1)};
		return __o;
	}
...
tink_web_routing_Router0.prototype = {
	...
	,root: function(ctx) {
		var ret = this.target.root().map(tink_core_Outcome.Success);
		return tink_core_Promise.next(ret.gather(),function(v) {
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(v)));
		});
	}
	,map: function(ctx) {
		return tink_core_Promise.next(new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(this.target.map()))),function(__data__) {
			if(ctx.accepts("application/json")) {
				var d = new tink_json_Writer0().write(__data__);
	...
	,spawnHero: function(ctx) {
		...
		default:
			var invalid = _g;
			tmp = new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(406,"Cannot process Content-Type " + invalid,{ fileName : "tink/web/macros/Routing.hx", lineNumber : 675, className : "tink.web.routing.Router0", methodName : "spawnHero"}))));
		}
		return tink_core_Promise.next(tmp,function(__body__) {
			var body = { name : __body__._0};
			return tink_core_Promise.next(new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(_gthis.target.spawnHero(body)))),function(__data__) {
				if(ctx.accepts("application/json")) {
					var d = new tink_json_Writer1().write(__data__);
					var this1 = "Access-Control-Allow-Origin".toLowerCase();
					return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(tink_web_routing_Response.textual(200,"application/json",d,[new tink_http_HeaderField(this1,"http://127.0.0.1:3000")]))));
				}
...
kro13_cosmith_data_GameDataFactory.GO_STUB_IMG = "images/go_stub.png";
kro13_cosmith_data_GameDataFactory.MAP_IMG = "images/map_test.jpg";
kro13_cosmith_data_scopes_MapData.TILE_SIZE = 10;
kro13_cosmith_server_Storage.goIds = 0;
...
kro13_cosmith_server_CosmithServer.main();
